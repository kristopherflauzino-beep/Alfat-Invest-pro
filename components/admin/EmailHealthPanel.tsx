...
export function EmailHealthPanel() {
  ...
  async function run(action: "verify" | "send_test") {
    setLoading(action);
    setMessage("");
    try {
      ...
      setMessage(action === "verify" ? "Conexão SMTP verificada." : "Mensagem aceita pelo provedor SMTP.");
      await refresh();
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      ...
      {configuration.errors.length > 0 && <div className="rounded-2xl bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200"><strong>Configuração de e-mail incompleta.</strong>{configuration.errors.map((item) => <p key={item} className="mt-1">{item}</p>)}</div>}
