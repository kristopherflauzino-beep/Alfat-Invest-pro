...
export function NotificationCenter() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(notificationsBody.notifications || []); setPreferences(preferencesBody.preferences || []);
  ...
  function updatePreference(topic: string, field: "inAppEnabled" | "emailEnabled" | "frequency", value: boolean | NotificationFrequency) {
    setPreferences((current) => current.map((item) => item.topic === topic ? { ...item, [field]: value } : item));
  }
  ...
  return <section className="space-y-5">
    ...
    {showPreferences && <div className="rounded-2xl border border-slate-200 p-4 dark:border-white/10"><div className="mb-4"><h3 className="font-black">Preferências por assunto</h3><p className="text-sm text-slate-500 dark:text-slate-300">Avisos essenciais de plano, pagamento, assinatura e segurança permanecem ativos.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead><tr className="border-b border-slate-200 dark:border-white/10"><th className="p-3">Assunto</th><th className="p-3">Na plataforma</th><th className="p-3">E-mail</th><th className="p-3">Frequência</th></tr></thead><tbody>{notificationTopics.map((topic) => { const preference = preferences.find((item) => item.topic === topic.topic); if (!preference) return null; return <tr key={topic.topic} className="border-b border-slate-100 dark:border-white/5"><td className="p-3 font-bold">{topic.label}{topic.essential && <span className="ml-2 rounded-full bg-cyan-500/10 px-2 py-1 text-[10px] uppercase text-cyan-700 dark:text-cyan-200">Essencial</span>}</td><td className="p-3"><input type="checkbox" checked={preference.inAppEnabled} disabled={topic.essential} onChange={(event) => updatePreference(topic.topic, "inAppEnabled", event.target.checked)} className="h-4 w-4" /></td><td className="p-3"><input type="checkbox" checked={preference.emailEnabled} disabled={topic.essential} onChange={(event) => updatePreference(topic.topic, "emailEnabled", event.target.checked)} className="h-4 w-4" /></td><td className="p-3"><select value={preference.frequency} disabled={topic.essential} onChange={(event) => updatePreference(topic.topic, "frequency", event.target.value as NotificationFrequency)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white">{Object.entries(frequencyLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></td></tr>; })}</tbody></table></div><button type="button" onClick={() => void savePreferences()} disabled={saving} className="mt-4 min-h-11 rounded-xl bg-cyan-500 px-5 py-2 font-black text-white disabled:opacity-60">{saving ? "Salvando..." : "Salvar preferências"}</button></div>}
    ...
    </div>    <p className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400"><Mail className="h-4 w-4" />O envio por e-mail respeita as preferências e a configuração SMTP da plataforma.</p>
  </section>;
