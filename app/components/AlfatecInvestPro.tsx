...
export default function AlfatecInvestPro() {
  ...
  const marketResults = useMemo(() => searchAssets(marketSearch, marketType, assets), [marketSearch, marketType, assets]);
            ...
            <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
              <PremiumCard title="Pesquisar ativos" description="Filtro por ticker, nome, setor, segmento e tipo." icon={Search}>
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                  ...
                  {marketResults.length === 0 && isTickerLike(marketSearch) && <button onClick={() => selectAsset(createGeneratedAsset(marketSearch))} className="rounded-3xl border border-teal-400 bg-teal-500/10 p-5 text-left font-bold text-teal-600">Adicionar ticker {normalizeTicker(marketSearch)} dinamicamente</button>}
                </div>
              </PremiumCard>
...
function AssetCard({ asset, favorites, onSelect, onFavorite }: { asset: Asset; favorites: string[]; onSelect: () => void; onFavorite: () => void }) {
  return <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5"><div className="flex items-start justify-between gap-3"><button onClick={onSelect} className="text-left"><p className="text-xs text-slate-500">{typeLabels[asset.type]} • {asset.segment}</p><h3 className="mt-1 text-xl font-black">{asset.ticker}</h3><p className="text-sm text-slate-500">{asset.name}</p></button><button onClick={onFavorite} className="rounded-xl bg-white p-2 dark:bg-white/10"><Star className={cls("h-4 w-4", favorites.includes(asset.ticker) && "fill-amber-400 text-amber-400")} /></button></div><div className="mt-4 grid grid-cols-3 gap-2"><InfoTile label="Preço" value={money.format(asset.price)} /><InfoTile label="Ano" value={pct(asset.changeYear)} /><InfoTile label="Score" value={`${asset.score}/100`} /></div></div>;
}
